...
exports.TableCellWithProperties = TableCellWithProperties;
exports.createReadTable = createReadTable;
/**
...
/**
 * Constructs a table cell with style properties. Is based on the original table cell.
 *
 ...
 */
function TableCellWithProperties(properties, children, colSpan) {
...
/**
 * Factory creating a reader of html tables from a docx openxml table fragments.
 *
 * @param {*} ReadResult return value wrapper result class
 * @param {*} emptyResult factory for an empty result
 * @param {*} readTableStyle reader for table styles
 * @param {*} combineResults helper function for combining attributes
 * @param {*} readXmlElements helper function to read xml elements
 * @param {*} calculateRowSpans helper function to calculate row spans
 *
 * @returns a read result with embedded html table
 */
function createReadTable(ReadResult, emptyResult, readTableStyle, combineResults, readXmlElements, calculateRowSpans) {
    function readTableWidth(element) {
        var width = element.children && element.children.find(function(child) {
            return child.name === "w:tblW";
        });
        if (width) {
            return new ReadResult({
                width: width.attributes["w:w"],
                widthUnit: width.attributes["w:type"]
            }, null, []);
        }
        return emptyResult();
    }
    ...
    function readTableProperties(element) {
        ...
        var styleAttributes = readTableStyle(element).map(function(style) {
            return {
        ...
        var widthAttributes = readTableWidth(element);
        if (widthAttributes.value) {
            attributes.push(widthAttributes);
        }
        return combineResults(attributes);
    }

    return function readTable(element) {
        var tablePropertiesReadResult = readTableProperties(element.firstOrEmpty("w:tblPr"));
        var tablePropertiesValue = tablePropertiesReadResult
            .value
            .reduce(function(acc, property) {
                return Object.assign(acc, property);
            }, {});

        var tableContents = readXmlElements(element.children)
            .flatMap(calculateRowSpans);
        var table = documents.Table(tableContents.value, tablePropertiesValue);
        return new ReadResult(table, null, Array.prototype.concat(tableContents.messages, tablePropertiesReadResult.messages));
    };
}
