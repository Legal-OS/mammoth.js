...
function DocumentConversion(options, comments) {
    ...
    function convertTableRow(element, messages, options) {
        ...
        var cssStyle = "";
        if  (element.height) {
            var parsedHeight = parseInt(element.height, 10);
            cssStyle += "height: ";
            cssStyle += isNaN(parsedHeight) ? "auto" : (parsedHeight / 20 + "pt") + ";";
        }
        var attributes = {};
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (element.borders && Object.keys(element.borders) !== 0) {
            cssStyle += ["top", "bottom", "left", "right"].reduce(function cssReducer(cssAccumulator, side) {
                if (element.borders[side] !== undefined && element.borders[side] !== undefined) {
                    ...
                    cssAccumulator += "border-" + side + "-width: " + (parseInt(borders.size, 10) / 8 || 1) + "pt;";

                    cssAccumulator += "padding-" + side + ": " + (parseInt(borders.space, 10) / 20 || 1) + "px;";
        ...
        if (element.width) {
            if (element.width.unit === "dxa") {
                cssStyle += "width: " + parseInt(element.width.value, 10) / 20 + "pt;";
            }
            if (element.width.unit === "pct") {
                cssStyle += "width: " + parseInt(element.width.value, 10) / 100  + "%;";
            }
        }
        if (cssStyle !== "") {
            attributes["style"] = cssStyle;
    ...
    function htmlPathForBreak(element) {
        ...
        if (style) {
            ...
            return htmlPaths.topLevelElement("br", { class: "PageBreak" })
        } else {
            return htmlPaths.empty;
        }
    }
