...
function DocumentConversion(options, comments) {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        var returnHtmlPath = null;
        if (style) {
            returnHtmlPath = style.to.clone();
            for (var i in returnHtmlPath._elements) {
                if (returnHtmlPath._elements[i].tagName == 'ol') {
                    if (element.numbering && element.numbering.format) {
                        returnHtmlPath._elements[i].attributes['type'] = listTypes[element.numbering.format];
                    }
                }
            }
        } else {
            ...
            returnHtmlPath = defaultParagraphStyle.clone();
        }
        var attributes = _.extend({}, (returnHtmlPath && returnHtmlPath._elements && returnHtmlPath._elements.length && returnHtmlPath._elements[0] && returnHtmlPath._elements[0].attributes) || {});
        ...
        if (element.indent) {
            if (element.indent.firstLine !== null && element.indent.firstLine !== undefined) {
                cssStyle = cssStyle + "text-indent: " + element.indent.firstLine / 20 + ";";
            }
            if (element.indent.hanging !== null && element.indent.hanging !== undefined) {
                cssStyle = cssStyle + "padding-left: " + element.indent.hanging / 20 + ";";
                cssStyle = cssStyle + "text-indent: -" + element.indent.hanging / 20 + ";";
            }
            if (element.indent.before !== null && element.indent.before !== undefined) {
                cssStyle = cssStyle + "margin-left: " + element.indent.before / 20 + ";";
            }
            if (element.indent.after !== null && element.indent.after !== undefined) {
                cssStyle = cssStyle + "margin-right: " + element.indent.after / 20 + ";";
            }
        }
        if (element.spacing) {
            ...
            if (element.spacing.before !== null && element.spacing.before !== undefined) {
                cssStyle = cssStyle + "margin-top: " + element.spacing.before / 20 + ";";
            }
            if (element.spacing.after !== null && element.spacing.after !== undefined) {
                cssStyle = cssStyle + "margin-bottom: " + element.spacing.after / 20 + ";";
            }
        }

        if (cssStyle && cssStyle !== "") {
            returnHtmlPath = returnHtmlPath.clone();
            returnHtmlPath._elements[0].attributes = _.extend(attributes, {
                style: (returnHtmlPath._elements[0].attributes && returnHtmlPath._elements[0].attributes.style || "") + cssStyle
            });
        }
        return returnHtmlPath;
    }
    ...
    function convertRun(run, messages, options) {
        ...
        if (run.fontSize) {
            paths.push(htmlPaths.element('span', { style: "font-size:" + run.fontSize }, { fresh: false }));
        }
    ...
    var defaultTablePath = htmlPaths.elements([
        htmlPaths.element("table", { style: "border-collapse: collapse;" }, { fresh: true })
    ]);
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (element.borders && Object.keys(element.borders) !== 0) {
            cssStyle = ["top", "bottom", "left", "right"].reduce(function cssReducer(cssAccumulator, side) {
                if (element.borders[side] !== undefined && element.borders[side] !== undefined) {
                    var borders = element.borders[side];
                    if (borders.lineType) {
                        cssAccumulator += "border-" + side + "-style: " + (lineTypeMapping[borders.lineType] || "solid") + ";";
                    }
                    if (borders.size) {
                        cssAccumulator += "border-" + side + "-width: " + (parseInt(borders.size, 10) / 20 || 1) + ";";
                    }
                    if (borders.space) {
                        cssAccumulator += "padding-" + side + ": " + (parseInt(borders.space, 10) / 20 || 1) + ";";
                    }
                    if (borders.color && element.borders.right.color !== "auto") {
                        cssAccumulator += "border-" + side + "-color: " + "#" + borders.color + ";";
                    }
                }
