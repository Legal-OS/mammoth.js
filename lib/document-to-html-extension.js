...
exports.withTableCellProperties = withTableCellProperties;
exports.withTableWidth = withTableWidth;
...
function withAlignmentIndentAndSpacing(htmlPath, element) {
    ...
    if (element.spacing) {
        if (element.spacing.line !== null && element.spacing.line !== undefined) {
            if (element.spacing.lineRule !== null && (element.spacing.lineRule === "atLeast" || element.spacing.lineRule === "exact")) {
...
/**
 * Extension supporting table cell properties.
 *
 * @param {htmlPaths.Element} element the element containing the style infos
 * @param {htmlAttributes} attributes the html attributes object to be extended with style infos
 */
function withTableCellProperties(element, attributes) {
    var cssStyle = "";
    var lineTypeMapping = {
        "nil": "none",
        "single": "solid",
    ...
    if (element.width) {
        ...
        if (element.width.unit === "pct") {
            cssStyle += "width: " + parseInt(element.width.value, 10) / 50 + "%;";
        }
    }
    if (cssStyle !== "") {
...
/**
 * Extension supporting table widths.
 *
 * @param {htmlPaths.Element} element the element containing the width properties
 * @param {htmlAttributes} defaultTablePath the html table path to be extended with table width styles
 *
 * @returns an updated html path array containing additional html table styles
 */
function withTableWidth(element, defaultTablePath) {
    var defaultTablePathAttributes = defaultTablePath._elements && defaultTablePath._elements[0] && defaultTablePath._elements[0].attributes;
    if (element.width && defaultTablePathAttributes) {
        if (element.widthUnit === "dxa") {
            var widthInPts = parseInt(element.width, 10) / 20;
            defaultTablePathAttributes.style += "width: " + widthInPts + "pt;";
        } else if (element.widthUnit === "pct") {
            var widthInPercent = parseInt(element.width, 10) / 50;
            defaultTablePathAttributes.style += "width: " + widthInPercent + "%;";
        }
    }
    return defaultTablePath;
}
