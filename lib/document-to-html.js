...
var extension = require("./document-to-html-extension");
...
function DocumentConversion(options, comments) {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            return extension.withAlignmentIndentAndSpacing(extension.withNumberings(style, element), element);
        } else {
            if (element.styleId) {
                messages.push(unrecognisedStyleWarning("paragraph", element));
            }
            return extension.withAlignmentIndentAndSpacing(defaultParagraphStyle.clone(), element);
        }
    }
    ...
    function convertRun(run, messages, options) {
        ...
        extension.withBackgroundColorAndFontStyle(paths, run);
    ...
    function getDefaultTable(element) {
        var attributes = {
        ...
        if (element.width) {
            if (element.widthUnit === "dxa") {
                var widthInPts =
                attributes.style += "width: " + widthInPts + "pt;";
            } else if (element.widthUnit === "pct") {
                var widthInPercent =  parseInt(element.width, 10) / 50;
                attributes.style += "width: " + widthInPercent + "%;";
            }
        }
        return htmlPaths.elements([htmlPaths.element("table", attributes, {fresh: true})]);
    }


    function convertTable(element, messages, options) {
        return findHtmlPath(element, getDefaultTable(element)).wrap(function() {
    ...
    function convertTableCell(element, messages, options) {
        ...
        extension.withTableCellProperties(element, attributes);
        return [
